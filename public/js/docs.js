...
async function renderSidebarFromChildHeaderForDocs(jsonPathHeader, jsonPathSidebar) {
  try {
    ...
    for (const item of items) {
      ...
      html += `
        <li class="toggleable ${openClass}">
          ${hasChildren ? '<span class="toggle-icon icon-chevron-docs"></span>' : ""}
          <a href="${item.path || item.file || "#"}" class="sidebar-item ${activeClass}">
            ${labelHTML}
          </a>
